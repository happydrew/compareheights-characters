...
function processFile(file, parentFolder, parentId, orderNum) {
    ...
    const mediaUrl = `https://raw.githubusercontent.com/happydrew/compareheights-characters_v2/refs/heads/main/${parentFolder}/${encodedFileName}`;
...
function processSubfolderFile(file, parentFolder, subfolderName, subId, parentId, orderNum) {
    ...
    // URL编码文件名以处理特殊字符
    const encodedFileName = encodeURIComponent(file);
    const mediaUrl = `https://raw.githubusercontent.com/happydrew/compareheights-characters_v2/refs/heads/main/${parentFolder}/${subfolderName}/${encodedFileName}`;
    const thumbnailUrl = mediaUrl;
